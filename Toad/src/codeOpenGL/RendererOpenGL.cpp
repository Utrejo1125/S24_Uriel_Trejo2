...
namespace Toad
{
	RendererOpenGL::RendererOpenGL()
	{
		...
		{
			TOAD_ERROR("Failed to initialize GLAD");
			return;
		}

		mShaderPtr = std::unique_ptr<Shader>{ new Shader{ "../Toad/Assets/Shaders/defaultVertexShader.glsl", "../Toad/Assets/Shaders/defaultFragmentShader.glsl" } };
	...
	void RendererOpenGL::Draw(Image& pic, int x, int y)
	{
		...
		mShaderPtr->Bind();
		mShaderPtr->SetUniform2Ints("ScreenSize", ToadWindow::GetWindow()->GetWidth(), ToadWindow::GetWindow()->GetHeight());
